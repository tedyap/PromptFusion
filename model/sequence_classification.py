...
class LinearWeightedSum(nn.Module):
    def __init__(self, n_inputs):
        ...
        self.weights = nn.ParameterList([nn.Parameter(torch.tensor([1/10])) for i in range(n_inputs)])
...
class RobertaPrefixFusionScalarForSequenceClassification(RobertaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
...
class RobertaPrefixFusionAttention1ForSequenceClassification(RobertaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
...
class RobertaPrefixFusionAttention2ForSequenceClassification(RobertaPreTrainedModel):
    ...
    def initialize_prompts(self, batch_size, prompt_n_head, pre_seq_len, n_embed):
        return torch.ones(pre_seq_len, batch_size, prompt_n_head * n_embed, device='cuda:0')
